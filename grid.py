...
class Grid:
    def __init__(self, size):
        ...
        pg.init()
        self.myfont = pg.font.SysFont('Arial', 50)  # Increased font size further
        self.screen_width = 800  # Increased screen width significantly 
        self.screen_height = 800  #
        self.cell_size = self.screen_width // self.size
        self.padding = 10  # Slightly increased padding
        self.colors = {
            0: (205, 193, 180),
            2: (238, 228, 218),
            4: (237, 224, 200),
        ...
        for i in range(self.size):
            for j in range(self.size):
                ...
                pg.draw.rect(self.screen, color, (j * self.cell_size + self.padding, i * self.cell_size + self.padding,
                                                  ...
                                                  border_radius=12)  # Increased rounded corners
                if value != 0:
                    # Adjust text size based on number length but with larger base sizes
                    text_size = 60 if value < 100 else 50 if value < 1000 else 40
    ...
    def step(self, action):
        ...
        if moved:
            ...
            # Check if target is reached (16384)
            for row in self.grid:
                if 16384 in row:
                    print("Congratulations! AI has reached the target of 16384!")
                    pg.quit()
                    quit()
